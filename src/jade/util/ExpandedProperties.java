 ...
 * variables (those usable from the OS shell). This class would need to be
 * carefully considered in different environments. This class relates to four
 * others as follows:
 * <ol>
 * <li>BasicProperties - This class provides the foundation class. It is
 * designed to be usable in the restrictive J2ME CLDC environment. It provides
 * enhanced property management as well as providing support for values
 * containing strings of the form <b><tt>${key}</tt></b>.
 ...
 * </pre>
 * 
 * <li>EnhancedProperties - Provides a concrete implementation of
 * ImportableProperties useable in the J2SE (desktop) world.
 * <li>PropertiesException - Extends RuntimeException and is thrown under
 * various error conditions by these classes.
 * 
...
public class ExpandedProperties extends EnhancedProperties {
	...
	public ExpandedProperties(String[] theArgs) {
		super();
		// System.out.println("ARGS:: "+theArgs[0]);
		parseArgs(theArgs);
		// System.out.println("ARGS:: "+theArgs[0]);
	}
	...
	/**
	 * Used to fetch environment properties and initialize environment properties
	 * object if this is first invocation.
	 * 
	...
	/**
	 * Fetch the system's environment properties and return them in an
	 * EnhancedProperties object. The fetching will be done using the appropriate
	 * shell command based on the OS. Based on method initially created by David
	 * Bell.
	 * 
	 ...
	 * <li>Move up one level in directory and search it as well as just first level
	 ...
	 *                          directory name where at which the search should
	 *                          ston.
	...
	public String locatePropertyFile(String aPropertyFileName, String aSearchCeiling) {
		...
		path = path.substring(0, path.length() - 2); // drop last separator and dot
		...
		do {
			...
			if ((searchCeilingFile != null) && (dir.compareTo(searchCeilingFile) == 0)) {
				break;
			}
	...
	/**
	 * Execute a command and return its output as a string. If the command generates
	 * multiple lines, each line will be separated using a separator obtianed from
	 * Java's System.getProperty("line.separator"). If the command fails in any way
	 * the result will be null.
	 * 
	 * @param aCommand       The command to run.
	 * @param isShellCommand If true, then the command will be prefixed with the
	 *                       appropriate system dependent shell invocation (cmd.exe,
	 *                       command.com, or /bin/sh/).
	...
	protected static String getCommandOutput(String aCommand, boolean isShellCommand) {
		...
		try {
			...
			while ((line = commandOutput.readLine()) != null) { // reads single full line
				if (lineCount > 0) {
					result.append(eol); // if multiple lines, separate with system separator
				}
			...
			success = (shell.exitValue() == 0); // final determination of success
	...
	/**
	 * Scan a directory for a property file and examine just first level of
	 * subdirectories.
	 * 
	 * @param aPropertyFileName Name of desired property file.
	 * @param aPath             Current full path.
	 * @param depth             Current depth - used to control recursive search
	 *                          depth of only first level subdirectories.
		...
		for (int i = 0; i < names.length; i++) {
			...
			if ((expandedSearch) && (depth == 0)) {
				...
				if (target.isDirectory()) {
					result = scanDirectory(aPropertyFileName, fullName, depth + 1);
			...
			if (fileName.equalsIgnoreCase(aPropertyFileName)) {
				result = (expandedSearch) ? testPropertyFile(fullName) : fullName;
	...
	/**
	 * Once the property file search has expanded beyond the current directory this
	 * method is used to examine property files which match the desired name - if it
	 * contains an import directive then return the value of that directive
	 * otherwise return the name of the property file. If any problem are
	 * encountered reading the property file return null.
	 * 
