 ...
 * @version $Date: 2005/02/23 10:46:39 $ $Revision: 2.42 $
 */
public class Boot {
	...
	private ContainerController containerCC = null;
	 ...
	 * 
	 * @param args The command line arguments. These use the form key:value or -key
	...
	public Boot(String[] args) {
		...
		try {
			...
			String[] argsNovo = prepareArgs(args);
			for (int i = 0; i < argsNovo.length; i++) {
				System.out.println("parametro Novo: " + argsNovo[i]);
			}

			profile = new BootProfileImpl(argsNovo); // qui adesso gli passa anche dbconf
		...
		try {
			...
			if (profile.getBooleanProperty(Profile.MAIN, true)) {
				containerCC = Runtime.instance().createMainContainer(profile);
			} else {
				containerCC = Runtime.instance().createAgentContainer(profile);
			}
	 ...
	 * </pre>
	 * 
	 * If the arguments contain either import:x or agents:x we will assume that the
	 * arguments are already in the new format and leave them alone. For "import:"
	 * we test if what follows is a file name and in the event it isn't we assume
	 * that it was if there are any other "-x" options following.
	 * <p>
	 * You can't mix the old form with the new as this would make the distinction
	 * between foo:bar as meaning a property named foo with a value bar or an agent
	 * named foo implmented by class bar impossible.
	 * <p>
	 * 
	...
	public void usage(PrintStream out) {
		...
		out.println(
				"  -mtp\t\t\tSpecifies a list, separated by ';', of external Message Transport Protocols to be activated.");
		out.println(
				"  \t\t\tBy default the HTTP-MTP is activated on the main-container and no MTP is activated on the other containers.");
		out.println("  -nomtp\t\tHas precedence over -mtp and overrides it.");
		out.println(
				"  \t\t\tIt should be used to override the default behaviour of the main-container (by default the -nomtp option unselected).");
		out.println(
				"  -aclcodec\t\tSpecifies a list, separated by ';', of ACLCodec to use. By default the string codec is used.");
	 ...
	 * them. It checks if the port number is a number greater than 0 otherwise it
	 * throws a BootException, and if the -nomtp has been set, then delete some
	 * other mtp wrongly set. If the user wants to start a platform the host must be
	 * the local host so if a different name is speficied it will be corrected and
	...
	protected void check() throws BootException {
		...
		try {
			...
			if (!isLocal) {
				throw new BootException("Error: Not possible to launch JADE a remote host ("
						+ properties.getProperty(profile.LOCAL_HOST) + "). Check the -host and -local-host options.");
			}
		} catch (UnknownHostException uhe) {
			throw new BootException("Error: Not possible to launch JADE an unknown host ("
					+ properties.getProperty(profile.LOCAL_HOST) + "). Check the -host and -local-host options.");
		}
	}
	 ...
	 * 
	 * // get user/pass from the user and // put them into the profile private void
	 * getUsernamePassword(SecurityFactory sf) {
	 * 
	 * // check if username:password has been provided String own =
	 * properties.getProperty(profile.OWNER); if (own != null) { String user = "";
	 * char[] passwd = new char[1]; // initial value to avoid java error;
	 * 
	 * // separate username:password int sep = own.indexOf(':'); if (sep > 0) { user
	 * = own.substring(0, sep); passwd = new char[ (own.length()) - sep];
	 * own.getChars(sep + 1, own.length(), passwd, 0); } else { // no ":" found user
	 * = own; if (sep == 0) user = ""; }
	 * 
	 * // if username or passwd have not been provided // ask for them try { if (
	 * (user.length() == 0)) { // ask for user:password jade.security.PwdDialog pwdD
	 * = sf.getPwdDialog(); pwdD.ask(); user = pwdD.getUserName(); passwd =
	 * pwdD.getPassword(); } else { if ( (sep < 0)) { // ask for the password
	 * jade.security.PwdDialog pwdD = sf.getPwdDialog(); pwdD.setUserName(user);
	 * pwdD.ask(); user = pwdD.getUserName(); passwd = pwdD.getPassword(); } // end
	 * if } // end if } catch (Exception e) { //} catch (jade.core.ProfileException
	 * e) { e.printStackTrace(); }
	 * 
	 * // set username:password profile.setParameter(profile.OWNER, user + ":" + new
	 * String(passwd)); } // end if (own != null) } // end getUsernamePassword()
	 */
